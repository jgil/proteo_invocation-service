...
module.exports = function(grunt) {
    ...
    var files = {
        '<%= proteo.dist %>/proteo.invocation-service/proteo.invocation-service.js': moduleFilesToConcat('<%= proteo.src %>/invocation-service'),
    };
    ...
    grunt.initConfig({
        ...
        concat: {
            ...
            dist: {
                ...
                dest: '<%= proteo.dist %>/proteo.invocation-service.js',
            },
        },
        ...
        uglify: {
            ...
            dist: {
                files: [{
                    ...
                    cwd: '<%= proteo.dist %>', // Src matches are relative to this path.
                    src: ['**/*.js'], // Actual pattern(s) to match.
                    dest: '<%= proteo.dist %>', // Destination path prefix.
        ...
        karma: {
            unit: {
                configFile: '<%= proteo.testsConfig %>/karma.unit.conf.js',
                autoWatch: false,
                singleRun: true
            },
            unitAutoWatch: {
                configFile: '<%= proteo.testsConfig %>/karma.unit.watch.conf.js',
                autoWatch: true
            },
            midway: {
                configFile: '<%= proteo.testsConfig %>/karma.midway.conf.js',
                autoWatch: false,
                singleRun: true
            },
        },
        ...
        protractor_coverage: {
            options: {
                configFile: '<%= proteo.testsConfig %>/protractor.e2e.conf.js',
                coverageDir: '<%= proteo.e2eCoverage %>',
                keepAlive: false,
                noColor: false,
                args: {},
            },
        ...
        makeReport: {
            src: '<%= proteo.e2eCoverage %>/*.json',
            options: {
                type: ['html', 'clover'],
                dir: '<%= proteo.e2eCoverage %>'
            }
        },
        ...
        instrument: {
            ...
            options: {
                lazy: true,
                basePath: "<%= proteo.e2eInstrumented %>"
            }
        },
        ...
        injector: {
            ...
            js: {
                files: {
                    '<%= proteo.demo %>/index.html': getAllFilesForDemo(files),
                }
            }
        },
        ...
        plato: {
            main: {
                ...
                files: {
                    '<%= proteo.reports %>/analysis/': [
                        '<%= proteo.src %>/**/*.js',
                        '<%= proteo.test %>/unit/**/*.js',
                        '<%= proteo.test %>/midway/**/*.js',
                        '<%= proteo.test %>/e2e/**/*.js',
                    ]
                }
            }
